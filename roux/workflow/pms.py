...
# %% auto 0
__all__ = ['extract_pms', 'expand_pms', 'read_pms', 'to_params', 'validate_pms', 'pre_params']
...
def to_params(
    cfg,        # pms
    alts=None,  # keys/values
    resolves=None,# values interpolation
    outp=None,
):
    """
    Get filled omegaconfig
    """
    if isinstance(cfg,str) and Path(cfg).exists():
        ## without interpolations
        cfg=read_dict(cfg,fmt='yaml')
    
    if alts is not None:
        from roux.workflow.cfgs import get_cfgs
        cfgs=get_cfgs(
            cfg,
            alts=alts,
            resolve=resolves is None,
        )
    else:
        cfgs={'':cfg}

    if resolves is not None:
        from roux.workflow.cfgs import resolve_cfg
        params={}
        for k,d in cfgs.items():
            ...
            for k2,v in params_.items():
                params[';'.join(([k] if k!='' else [])+[k2])]=v
    else:
        params=cfgs
    if outp is not None:
        to_dict(
